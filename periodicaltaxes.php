...
if((!empty($category)&&!empty($busid)&&!empty($from) && !empty($to!=0) )&& ( ($_POST["quartertax"])||($_POST["fixness"])||($_POST["eibalteration"])||($_POST["termination"])||($_POST["rtacon"])||($_POST["rtapegreen"])||($_POST["tollway"] ) )){
...
if($conn->connect_error)
{
  die("Connection Failed:".$conn->connect_error);
   ...
   $query="INSERT INTO periodicaltaxes  VALUES ('$category','$busid','$from','$to','$quartertax','$fixness','$eibalteration','$termination','$rtacon','$rtapegreen','$tollway')";
   $rs=mysqli_query($conn,$query);  
   if($rs){
      echo '<script> alert("Details submitted successfully");  </script>';
   }
   else{
    echo '<script> alert("Error while uploading detials!!");  </script>';
   }
}
}
}
...
<body>
    ...
    <form action="" method="post">
        <div class="myDiv">
            ...
            <label>From</label><input type="date" name="from" min="2007-01-01" id="txt"></br></br>
            <label>To</label><input type="date" name="to" max="2090-01-01" id="txt"></br></br>
            <label>Quarter tax</label> <input type="number" name="quartertax" id="txt" /></br></br>
            <label>Fixness</label> <input type="number" name="fixness" id="txt" /></br></br>
            <label>EIB Alteration</label> <input type="number" name="eibalteration" id="txt" /></br></br>
            <label>termination</label> <input type="number" name="termination" id="txt" /></br></br>
            <label>RTA CON & transfer </label><input type="number" name="rtacon" id="txt" /></br></br>
            <label>RTA pe & green</label> <input type="number" name="rtapegreen" id="txt" /></br></br>
            <label>Toll Way </label><input type="number" name="tollway" id="txt" /></br></br></br>
            <a>
...
$fromdate=$_POST["from"];
  ...
  if($conn->connect_error){
   ...
   if($busid!=""){
          $query=mysqli_query($conn,"select * from periodicaltaxes where busid='$busid' and `FromDate` between '$fromdate' and '$todate'");
        }
else{
          $query=mysqli_query($conn,"select * from periodicaltaxes where  `FromDate` between '$fromdate' and '$todate'");
        }
...
<tr>
    ...
    echo '<script type="text/javascript"> alert("Please fill fromdate and todate")</script>';
}}
if(isset($_GET["delete"])) {
    $conn = mysqli_connect("localhost", "root", "", "transport");
    $delete_id = $_GET["delete"];
    $date = $_GET["date"];
    $res = mysqli_query($conn, "DELETE FROM periodicaltaxes WHERE busid='$delete_id' and `FromDate`='$date'");
    if($res){
        echo "<script type='text/javascript'> alert('Record Deleted Successfully'); window.location.href = periodicaltaxes.php';</script>";
    }else{
