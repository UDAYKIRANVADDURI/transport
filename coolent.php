...
if(isset($_POST['insert'])) {
    ...
    if(!empty($busid) && !empty($cate) && !empty($date) && !empty($cost))
    {
        ...
        {
        //id is present in masterdemo table, insert values into coolent table
        $sql = "INSERT INTO `coolent`(`id`, `category`, `dates`, `tcost`) VALUES ('$busid','$cate','$date','$cost')";
...
<body>
    ...
    <form action="" method="post">
        <div class="myDiv">
            ...
            <label for="tcost">Cost</label><input type="number" name="cost" id="txt" /><br><br>
            <a>
...
if(!empty($date) && !empty($to_date)){
    if($busid==''){
        ...
        $qry=mysqli_query($conn,"SELECT * from coolent WHERE dates BETWEEN '$date' and '$to_date'");
    }else{

        //query if input is busid and dates.

        $qry=mysqli_query($conn,"SELECT * from coolent WHERE id='$busid' and dates BETWEEN '$date' and '$to_date'");
    }
...
if(isset($_GET["delete"])) {
    ...
    $date=$_GET["date"];
    $res = mysqli_query($conn, "DELETE FROM coolent WHERE id='$delete_id' and dates='$date'");
    if($res){
        echo "<script type='text/javascript'> alert('Record Deleted Successfully'); window.location.href = 'coolent.php';</script>";
    }else{
