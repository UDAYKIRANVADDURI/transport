...
if(!empty($busid) && !empty($cate) && !empty($date) && !empty($cost))
{
    ...
    {
    $sql = "INSERT INTO `gearoil`(`id`, `category`, `dates`, `tcost`) VALUES ('$busid','$cate','$date','$cost')";
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <a>
                <button type="submit" value="Submit" name="insert"
                    class="btn">Submit</button><span>&emsp;&emsp;&emsp;&emsp;&emsp;</span>
...
if(!empty($date) && !empty($to_date)){
    if($busid==''){
        ...
        $qry=mysqli_query($conn,"SELECT * from gearoil WHERE dates BETWEEN '$date' and '$to_date'");
    }else{

        //query if input is busid and dates.
        
        $qry=mysqli_query($conn,"SELECT * from gearoil WHERE id='$busid' and dates BETWEEN '$date' and '$to_date'");
    }
...
if(isset($_GET["delete"])) {
    ...
    $date = $_GET["date"];
    $res = mysqli_query($conn, "DELETE FROM gearoil WHERE id='$delete_id' and dates='$date'");
    if($res){
        echo "<script type='text/javascript'> alert('Record Deleted Successfully'); window.location.href = 'gearoil.php';</script>";
    }else{
