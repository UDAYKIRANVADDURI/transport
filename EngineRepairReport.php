...
$query=mysqli_query($db,"insert into `enginerepairreport` select m.busid,e.category,e.description,sum(e.Cost) Cost  from `masterdemo` m left OUTER JOIN `enginerepairs` e on m.busid=e.busid and e.fromdate BETWEEN '$fromdate' and '$todate' group by m.busid ; ");
$query2=mysqli_query($db,"insert into `enginerepairreport` select 'Total' as busid , '' , '' , sum(Cost) from `enginerepairreport`");
    
header("Content-Type: application/octet-stream"); 
header("Content-Disposition: attachment; filename=\"enginerepair.xls\"");
...
$excelData = "<table border=2px solid black;><tr><td colspan='2' align='center'><b>Engine Repair Report ( $fromdate to $todate )</b></td></tr>\n";
...
<html>
    <head>
    <title>ENGINE REPAIRS REPORT</title>
...
//  fromdate between '$fromdate' and '$todate'
    ...
    $query=mysqli_query($db,"insert into `enginerepairreport` select m.busid,e.category,e.description,sum(e.Cost) Cost  from `masterdemo` m left OUTER JOIN `enginerepairs` e on m.busid=e.busid and e.fromdate BETWEEN '$fromdate' and '$todate' group by m.busid ; ");
    $query2=mysqli_query($db,"insert into `enginerepairreport` select 'Total' as busid , '' , '' , sum(Cost) from `enginerepairreport`");
    $query3 = mysqli_query($db,"select * from enginerepairreport;"); 
    echo "<table >
    <tr>
    <th colspan='2'>Engine Repairs Report from <br>(" . date("j-F-Y", strtotime($fromdate)) . " to " . date("j-F-Y", strtotime($todate)) . ")</th></tr><tr>
...
$query4 = mysqli_query($db,"delete from enginerepairreport;");
